...
public class WatermarkService {
    ...
    public WatermarkService(UtilService utilService, WaterMarkMapper waterMarkMapper, UserMapper userMapper) {
    ...
    //-----------------------Embed-------------------------//
    public ResponseEntity<byte[]> embed(WatermarkEmbed watermarkData, MultipartFile imgFile, HttpServletRequest request, HttpServletResponse response) throws IOException {
        if(watermarkData == null){
            throw new CustomExceptions.MissingRequestBodyException("Watermark data is missing");
        }
        if(StringUtils.isBlank(watermarkData.getText())){
            throw new CustomExceptions.MissingRequestBodyException("Text is missing");
        }
        String username = extractUsernameFromSecurityContextOrCookie(request);
        if(!watermarkData.getUsername().equals(username)){
            throw new CustomExceptions.InvalidRequestException("Username is not equals");
        }

        // API 키 유효성 검사
        Optional<UsersApiKey> userApiKey = userMapper.FindByApiKey(username);
        if(!userApiKey.isPresent()){
            throw new CustomExceptions.InvalidRequestException("Apikey is not found");
        }

        UsersApiKey apiKeyInfo = userApiKey.get();
        if(!apiKeyInfo.isApiEnabled()){
            throw new CustomExceptions.InvalidRequestException("Apikey is disabled");
        }

        if(!watermarkData.getApikey().equals(apiKeyInfo.getApiKey())){
            throw new CustomExceptions.InvalidRequestException("Apikey is not equals");
        }

        String text = watermarkData.getText();
        Random random = new Random();
        String token = hashText( 6 + random.nextInt(2));
    ...
    public ResponseEntity<WatermarkLog> decode(MultipartFile imgFile, WatermarkDecode watermarkData, HttpServletRequest request, HttpServletResponse response) throws IOException {
        // 1. 파일 입력 체크
    ...
    public ResponseEntity<WatermarkLog> decodeVIP(MultipartFile imgFile, WatermarkEmbed watermarkData, HttpServletRequest request, HttpServletResponse response) throws IOException {
        // 1. 파일 입력 체크
        if (imgFile == null || imgFile.isEmpty()) {
            throw new CustomExceptions.MissingRequestBodyException("Image file is missing");
        }
...
//        validateApiKey(username, watermarkData.getApikey());
