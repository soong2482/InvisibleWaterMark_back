...
public class WatermarkController {
    ...
    @PostMapping("${Security.backEndPoint}/User/DecodeWaterMark")
    public ResponseEntity<WatermarkLog> decodeWatermark(
            @RequestPart(value = "imgfile", required = false) MultipartFile imgFile,
            HttpServletRequest request,
            HttpServletResponse response
    ) throws IOException {
        return  watermarkService.decode(imgFile, request, response);
    }
    ...
    public ResponseEntity<WatermarkLog> decodeWatermarkVIP(
        ...
        return  watermarkService.decodeVIP(imgFile, request, response);
    }
}